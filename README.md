...
- [turva.dev](https://turva.dev/) - Agent-readiness audits for websites, APIs and Shopify storefronts. Registers three tools through `navigator.modelContext.provideContext()` on every content page: `get_services` returns the services page as markdown with machine-readable pricing, `get_company` returns the business record, and `get_contact` returns the async contact channels.
