...
- [turva.dev](https://turva.dev/) - Agent-readiness audits for websites, APIs and Shopify storefronts. Registers six tools through `navigator.modelContext.provideContext()` on every content page. `search_content` reads the published llms.txt index, `get_page` returns any page of the site as markdown and refuses a path that resolves to another origin, `open_page` navigates to one of ten named pages instead of a caller-supplied URL, and `get_services`, `get_company` and `get_contact` return the priced catalogue, the business record and the async contact channels.
