...
- [Deveserisso](https://deveserisso.com.br/) - Brazilian pop-culture portal covering movies, TV series, and books. Exposes five WebMCP tools (`search`, `get_post`, `list_categories`, `get_recent_posts`, `subscribe_newsletter`) via `document.modelContext.registerTool()` on every page, so in-browser agents can search articles, read a full post, browse categories, list recent posts, and subscribe to the weekly newsletter directly on the production site.
